...
module.exports = function (app) {
	app.get("/messages", auth, (req, res) => {
		...
		client.execute(query, function(err, result) {
			...
			result.rows.map((row) => {
				row.view_link = ("/bulkSMS/messages/" + row.id)
				row.edit_link = ("/bulkSMS/messages/edit/" + row.id),
				row.delete_link = ("/bulkSMS/messages/delete/" + row.id)
			})
			...
			res.render('bulkSMS/messages/list', renderData);
		});
	...
	app.route("/messages/new")
		.get(function(req, res) {
			res.render('bulkSMS/messages/new', {
				layout: "bulkSMS",
				session: req.session,
				status: "Online",
				page: "Registration",
